...
if (!RECEIVER_URL) {
  ...
  console.error('  npm run submit:rich');
  process.exit(1);
}
...
const fixturePath = join(__dirname, '..', 'docs', 'examples', 'mindloom-rich-schema-report.json');
const body = readFileSync(fixturePath, 'utf-8');

const endpoint = RECEIVER_URL.replace(/\/$/, '') + '/api/mindloom/reports';
console.log('Sending to:', endpoint);
console.log('Fixture:   mindloom-rich-schema-report.json (rich format)');
...
if (data.ok && data.report_url) {
  ...
  console.log('Откройте report_url — должны отображаться все секции rich-формата на русском языке.');
} else if (!data.ok) {
  console.error('\nError:', data.error);
  process.exit(1);
}
