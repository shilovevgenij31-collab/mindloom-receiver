...
if (!RECEIVER_URL) {
  ...
  console.error('  npm run submit:intake:file');
  process.exit(1);
}
...
const samplePath = join(__dirname, '..', 'docs', 'examples', 'intake-file-sample.json');
const body = readFileSync(samplePath, 'utf-8');

const endpoint = RECEIVER_URL.replace(/\/$/, '') + '/api/mindloom/intake';
console.log('Sending to:', endpoint);
console.log('Fixture:   intake-file-sample.json (file_extracted_text)');
...
if (data.ok && data.admin_url) {
  ...
  console.log('Откройте admin_url — должен отображаться чип "файл", filename, charCount и полный текст.');
} else if (!data.ok) {
  console.error('\nError:', data.error);
  process.exit(1);
}
